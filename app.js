...
// === Meta helpers med fallback (IDB -> localStorage) ===
...
window.addEventListener('load', ()=>{
  ...
  // Låsskärm
  $('#setPassBtn').onclick = ()=>setInitialPass(passEl.value);
  $('#unlockBtn').onclick  = ()=>unlock(passEl.value);
  $('#wipeLocalOnLock').onclick = wipeAll;

  // CRUD
  $('#newEntryBtn').onclick = ()=>{ state.currentId=null; editor.innerHTML=''; dateLine.textContent=''; editor.focus(); };
  ...
  $('#colorBtn').oninput     = e=>document.execCommand('foreColor',false,e.target.value);
  ...
  $('#wipeLocalBtn').onclick = wipeAll;

  // Font
  const fs = $('#fontSelect');
  const saved = localStorage.getItem('rd_font');
  if(saved){ editor.style.fontFamily = saved; fs.value = saved; }
  fs.onchange = e=>{ editor.style.fontFamily = e.target.value; localStorage.setItem('rd_font', e.target.value); };
