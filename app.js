  ...
  if(currentUser){
    try{
      await supabase.from('entries').upsert({
        id:rec.id, user_id:currentUser.id, title:rec.title, created:rec.created, updated:rec.updated, cipher:rec.cipher
      });
    }catch{
      await DB.queue('upsert', rec); // offline → outbox
    }
  }
}
...
async function deleteCurrent(){
  ...
  if(currentUser){
    try{
      await supabase.from('entries').delete().eq('id', id).eq('user_id', currentUser.id);
    }catch{
    ...
    if(currentUser){
      ...
      if(Array.isArray(data)){
        for(const s of data){
          ...
          if(!local || (s.updated||0) > (local.updated||0)){
            await DB.put('entries', s);
          }
        }
        state.entries = await DB.all();
      }
...
async function importAll(file){
  ...
  for(const e of json.entries){
    ...
    if(currentUser){
      try { await supabase.from('entries').upsert({ ...e, user_id: currentUser.id }); }
      catch { await DB.queue('upsert', e); }
    }
  }
  state.entries = await DB.all();
  renderList();
...
// Normalisera återställningskod (tål mellanslag/streck/små bokstäver)
function normalizeRC(s){
  return (s || '').toUpperCase().replace(/[^A-Z2-9]/g, '');
}

function showRecovery(rcPretty){
  $('#recoveryCode').value = rcPretty;
  $('#recoveryDialog').showModal();
}

async function getOrCreateRecovery(){
  let rcPretty = await DB.getMeta('recovery_code');
  if(!rcPretty){
    if(!state.dek) return alert('Lås upp först.');
    rcPretty = await regenRecoveryWrap(state.dek); // skapar ny och sparar
  }
...
// Bakåtkompatibel: prova normaliserad OCH exakt inmatning
async function loadDEK_withRecoveryCompat(rcInput){
  const wrap = await DB.getMeta('wrap_recovery');
  if(!isValidWrap(wrap)) throw new Error('Skadad nyckelmetadata (wrap_recovery)');

  const candidates = [ normalizeRC(rcInput), rcInput.trim() ];
  let lastErr;
  for(const c of candidates){
    try{
      const k = await CryptoBox.pbkdf2(c, fromHex(wrap.saltHex));
      return await CryptoBox.aesDecryptRaw(k, wrap.payload);
    }catch(e){ lastErr = e; }
  }
  throw new Error('Fel återställningskod.');
}

async function applyReset(){
  const rcRaw = $('#rcInput').value;
  const p1 = $('#newPass1').value, p2 = $('#newPass2').value;
  const out = $('#resetStatus');

  if(!rcRaw || !p1 || p1 !== p2){
    out.textContent = 'Kontrollera koden och att lösenorden matchar.';
    return;
  }

  try{
    const dek = await loadDEK_withRecoveryCompat(rcRaw);
    await rewrapPass(p1, dek);
    state.dek = dek; state.pass = p1;
    out.textContent = 'Dagbokslösen uppdaterat. Du kan nu låsa upp med det nya.';
  }catch(err){
...
async function bioRegister(){
  ...
  // Kapsla DEK lokalt för snabb unlock
...
async function bioDisable(){
  ...
  localStorage.removeItem('bio_capsule');
...
async function bioUnlock(){
  ...
  try{
    ...
    state.dek = new Uint8Array(dekBuf);
    state.pass = null;
    ...
    statusEl.textContent = 'Biometrisk upplåsning avbröts/misslyckades.';
  }
}
...
function renderIconTab(tab){
  const grid = $('#iconGrid');
  grid.innerHTML = '';
  (ICON_SETS[tab]||[]).forEach(ch=>{
...
function activateTab(btn){
  document.querySelectorAll('#iconTabs .tab').forEach(t=>t.classList.remove('active'));
  btn.classList.add('active');
  renderIconTab(btn.dataset.tab);
}

function openIconPalette(){
  const dlg = $('#iconDialog');
  if(!dlg.dataset.ready){
    document.querySelectorAll('#iconTabs .tab').forEach(b=>b.addEventListener('click', ()=>activateTab(b)));
    dlg.dataset.ready='1';
  }
  const active = document.querySelector('#iconTabs .tab.active') || document.querySelector('#iconTabs .tab');
...
async function signUp(){
  ...
  const pwd=$('#acctPass').value;
  $('#authStatus').textContent='Skapar konto...';
  const { error } = await supabase.auth.signUp({ email, password: pwd });
  $('#authStatus').textContent = error ? error.message : 'Konto skapat. Kolla ev. verifikationsmail.';
}
...
async function signIn(){
  ...
  $('#authStatus').textContent='Inloggad.';
}
...
window.addEventListener('load', async ()=>{
  ...
  // Återställ session om inloggad sedan tidigare
